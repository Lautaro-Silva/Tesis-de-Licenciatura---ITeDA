...
CAMBIE COMO ES EL FITEO Y SAQUE EL A0 PERO NO ANDA ESE SCRIPT (FACILMENTE RESOLUBLE)
"""
...
def add_heading(pdf, text, level=1):
    ...
    if level == 1:
        # Evitar saltos de página innecesarios si hay mucho espacio
        if pdf.get_y() > 200:
            pdf.add_page()
        else:
            pdf.ln(10)
...
def add_plot(pdf, fig=None):
    ...
    # --- Chequeo de espacio en la página ---
    # Altura de la imagen (asumiendo aspect ratio 4:3 aprox)
...
# --- ❗️ 1. FUNCIÓN DE FIT (CORREGIDA, sin A0) ❗️ ---
def fit_func_deg(phi_deg, A1, phi0_deg):
    """
    A1 = Amplitud de la asimetría (el 'A' que buscás)
    phi0_deg = Fase (dónde está el máximo, en grados)
    """
    # A0 (normalización) está fija en 1.0
...
def run_analysis(folder_path, pdf):
    ...
    add_body_text(pdf, f"DataFrame 'df_new' (módulos) cargado con {total_modules:,} filas.")
    ...
    add_body_text(pdf, f"DataFrame 'df_events' (lluvias) creado con {total_events:,} filas únicas.")
    add_dataframe(pdf, df_events.head(), title="df_events.head()")
    
    # --- Volver a la Portada para añadir stats ---
    pdf.set_page(1)
    ...
    umbral_E = 0.9 # Umbral para outliers
    ...
    plt.legend()
    plt.grid(True)
    plt.ylim(17.0, Y_full.max() + 0.5) # Y-lim dinámico
    ...
    phi_plot_bins = 24 # ❗️ Usamos 24 bins como en tu penúltimo script
    ...
    for i in range(N_theta_bins):
        ...
        try:
            # ❗️ CAMBIO: p0 ahora solo tiene 2 parámetros (A1, phi0)
            popt_umd, pcov_umd = curve_fit(fit_func_deg, x_umd, y_umd, p0=[0.05, 0.0], sigma=yerr_umd)
            # ❗️ CAMBIO: popt ahora es [A1, phi0]
            A1_umd, A1_err_umd = popt_umd[0], np.sqrt(pcov_umd[0,0])
            phi0_umd_deg = popt_umd[1]
            ...
            fit_results.append({
                ...
                'A1': A1_umd, 'A1_err': A1_err_umd, 'phi0_deg': phi0_umd_deg,
        ...
        try:
            # ❗️ CAMBIO: p0 ahora solo tiene 2 parámetros (A1, phi0)
            popt_sd, pcov_sd = curve_fit(fit_func_deg, x_sd, y_sd, p0=[0.05, 0.0], sigma=yerr_sd)
            # ❗️ CAMBIO: popt ahora es [A1, phi0]
            A1_sd, A1_err_sd = popt_sd[0], np.sqrt(pcov_sd[0,0])
            phi0_sd_deg = popt_sd[1]
            ...
            fit_results.append({
                ...
                'A1': A1_sd, 'A1_err': A1_err_sd, 'phi0_deg': phi0_sd_deg,
...
def main():
    ...
    output_filename = f"Reporte_{base_name}_v2.pdf"
    ...
    # --- Carga de datos ANTES de la portada ---
    # (Esto es necesario para que la portada tenga el resumen)
    try:
        ...
        # Ejecutar el análisis (pasando el df ya cargado)
        run_analysis(df_new, pdf) # ❗️ CAMBIO: Pasamos el DF

    except Exception as e:
        print(f"\nERROR DURANTE LA EJECUCIÓN DEL ANÁLISIS:", file=sys.stderr)
        print(traceback.format_exc(), file=sys.stderr) # Imprime el traceback completo
