...
def run_analysis(df_new, pdf):
    ...
    sc = ax.scatter(
        df_plot["r_core"], df_plot["theta_REC"], df_plot["nMuones_REC"],
        c=df_plot["nMuones_REC"], cmap="viridis", alpha=0.7, s=10
    )
    ...
    for i in range(N_theta_bins):
        ...
        Z, _, _ = np.histogram2d(
            ...
            bins=[r_bins, phi_edges], weights=df_bin['nMuones_REC']
        )
