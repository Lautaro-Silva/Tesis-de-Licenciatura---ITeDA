...
Por otro lado, la asimetría de los muones registrados por el SD exhibe un comportamiento transicional. Es aquí donde radica el mayor hallazgo analítico de esta comparación dado que el SD registra muones superficiales que, en los primeros bines radiales, se comportan de manera análoga al UMD (con una asimetría positiva creciente). Sin embargo, al acercarse a bines de mayor radio ($\sim 600$~m), la fenomenología cambia drásticamente y la magnitud de asimetría comienza a decrecer, invirtiendo su polaridad hacia valores fuertemente negativos en el \textit{far-core}. Este fenómeno condiciona por completo a la señal total del SD, evidenciando que, a grandes distancias del eje, la asimetría del tanque Cherenkov está dominada por la población muónica de superficie.

Para comprender el origen físico de esta inversión, es necesario recurrir a la cinemática de propagación de la cascada. Como fue demostrado por la Colaboración Auger \cite{Luce_ICRC2021}, el perfil azimutal muónico es el resultado de la competencia entre dos fenómenos: la atenuación atmosférica y la dispersión geométrica del haz. El mecanismo subyacente a esta dispersión se explica al recuperar la cinemática de producción detallada en la Sección \ref{subsubsec:divergencia}. Como se estableció en el modelo de Cazón \cite{Cazon2012}, la distancia radial a la que impacta un muón está gobernada por la relación inversa con su energía longitudinal ($r \propto p_T c / E$), de modo que el ángulo de divergencia de salida resulta $\alpha \approx p_T c / E$. Esto implica que el espectro muónico de baja energía (muones blandos) adquiere ángulos de deflexión significativamente mayores respecto al eje de la lluvia. Dado que la señal a nivel del suelo medida por el SD está dominada numéricamente por esta población blanda, el detector se vuelve extremadamente sensible a dicha dispersión geométrica.

En lluvias inclinadas, la topología de incidencia impone que la proyección del disco sobre el plano del arreglo no sea simétrica. Siguiendo la dinámica de proyección \cite{Cazon2012}, la región temprana de la lluvia se estira sobre el suelo, mientras que la región tardía se comprime ($\alpha_{\mathrm{temprano}} > \alpha_{\mathrm{tardio}}$). Al estar el haz superficial dominado por muones blandos de gran dispersión angular, esta compresión geométrica genera que la región tardía concentre una porción intrínsecamente más densa del haz original. A grandes distancias del núcleo, esta acumulación artificial supera holgadamente el vaciamiento producido por la atenuación atmosférica, forzando la asimetría hacia el régimen negativo. En consecuencia, la inversión observada en el SD Total es el resultado determinista de convolucionar un frente electromagnético temprano con este fondo muónico superficial dominado por la distorsión geométrica.
...
La comparación con el UMD valida empíricamente este modelo analítico. La barrera de atenuación pasiva actúa como un filtro umbral estricto, suprimiendo por completo a la población blanda y permitiendo únicamente el paso de muones altamente energéticos ($E \gtrsim 1\text{ GeV}$). De acuerdo a la relación cinemática, estos muones viajan en un haz mucho más estrecho y colimado. Al neutralizarse el ensanchamiento radial extremo, el efecto asimétrico de compresión/estiramiento del disco pierde dominancia espacial. De este modo, la atenuación longitudinal recupera el control del observable, lo que justifica la robusta y monótona asimetría positiva observada en la curva del detector subterráneo.
...
\subsection{Modelado Fenomenológico Analítico-Monte Carlo del Filtro Cinemático}

Si bien el análisis de simulaciones con el framework \textit{Offline} ha demostrado de manera contundente la degradación sistemática de la asimetría azimutal en el Detector de Superficie (SD) y su preservación en el Detector Subterráneo de Muones (UMD), el origen íntimo de esta competencia fenomenológica requiere un abordaje de primeros principios. Las simulaciones estándar empaquetadas (ADST) carecen de la granularidad necesaria para rastrear la cinemática de producción individual de cada partícula antes de su propagación por el medio.

Como extensión natural y fundamental de este trabajo, se propone el desarrollo de un propagador \textit{Fast-Monte Carlo} que acople la producción hadrónica exacta con el transporte analítico de los muones. La hoja de ruta para este modelo fenomenológico se estructura en las siguientes etapas:

\begin{enumerate}
	\item \textbf{Extracción del Espectro de Producción Original:} Se generarán simulaciones dedicadas en CORSIKA sin el proceso de difuminado topográfico del detector. El objetivo es extraer directamente la función de distribución tridimensional en el punto de decaimiento del mesón progenitor: $F(X, E_i, p_T)$, donde $X$ es la profundidad atmosférica de generación, $E_i$ la energía inicial y $p_T$ el momento transversal heredado.
	
	\item \textbf{Propagación Analítica y Transporte:} En lugar de utilizar una Función de Distribución Lateral (LDF) genérica y simétrica, se propagará cada muón hacia el suelo utilizando el modelo de transporte cinemático formalizado por L. Cazón et al. (2012). Esta etapa aplicará las pérdidas continuas de energía por ionización ($dE/dX$) según la isoterma atmosférica local y la probabilidad de supervivencia por decaimiento ($W_{\mathrm{decay}}$), acopladas al mapeo del ángulo sólido que rige la divergencia de la cascada.
	
	\item \textbf{Implementación de Umbrales Instrumentales:} Al obtener la energía final residual a nivel del suelo ($E_f$), se aplicarán funciones escalón para emular los diferentes detectores: un umbral blando ($E_f > 0.3$ GeV) para la señal Cherenkov del SD, y un umbral duro ($E_f > 1.0$ GeV) correspondiente al blindaje pasivo de 2.3 m de tierra del UMD.
\end{enumerate}

Este marco de trabajo permitirá demostrar analíticamente cómo el espectro de baja energía (altamente divergente) infla de manera espuria el Jacobiano de proyección espacial, forzando la asimetría negativa ($A_1 < 0$) en el régimen \textit{far-core} del SD. Simultáneamente, probará desde las ecuaciones de transporte que la barrera de atenuación del UMD filtra las trayectorias de bajo momento longitudinal, anulando la divergencia geométrica y revelando únicamente la asimetría positiva intrínseca al desarrollo longitudinal de la lluvia.
