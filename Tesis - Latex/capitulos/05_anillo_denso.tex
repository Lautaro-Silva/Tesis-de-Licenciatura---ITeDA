...
En principo se analizo si los ajustes realizados para obtener los valores de asimetria $A_1$ convergian de manera razonable. Para esto se estudio el factor de bondad de ajuste $\chi^2_\nu$ para todos los datos mostrados en la Figura \ref{fig:evolucion_asym_vs_theta}. Estos valores se pueden ver en la Figura \

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{capitulos/imagenes_capitulos/cap5/muon_counting_validation.pdf}
	\caption{Resido Relativo de la reconstruccion de muones comparado con los inyectados para todas las lluvias de protones realizadas con el modelo \texttt{Sibyll 2.3e} en el anillo denso. Se obtuvo una media casi cero con una resolución pequeña en valor absoluto.}
	\label{fig:bias_umd_global}
\end{figure}

Se analizo como era el bias de la reconstrucción en al cantidad de muones analizando lso residuos de manera que

\begin{equation}
	\text{Residuo Relativo de Muones} = \frac{N^{REC}_\mu - N^{MC}_\mu}{N^{MC}_\mu}
\end{equation}

donde esta metrica permite analziar la proporcion de muones reconstrudios en comparacion con los inyectados, pero normalizada. Este estudio se puede ver en la Figura \ref{fig:bias_umd_global}
...
Vale la pena destacar que, aun cuando la distribución tiene una media casi cero con una buena resolución, se puede ver que la distribución no es simétrica respecto del cero. Esto es esperado dado que jamas se puede tener menos que el 100\% de los muones inyectados como reconstruidos, pero si se puede tener una mayor cantidad (como se ve en la Figura que ocurre que a veces se reconstruyen hasta 4 veces más muones que los inyectados). 

Se intento ver si esta asimetria de la distribucion respecto del cero podia ser la causa de las diferencias entre la ampl;itud de asimetrai $A_1$ entre los datos REC y los datos MC. Para esto se construyo un modelo que a los datos MC se les sampleaba un valor random obtendio de la distribucion encontrada en la Figura \ref{fig:bias_umd_global} y se le restaba 
