...
\subsubsection{Interpretación Física: dependencia con el $X_{max}$}
...
% --- AGREGAR AL FINAL DEL CAPÍTULO 5 ---

\subsubsection{Contraste fenomenológico con el Detector de Superficie}
\label{subsec:contraste_sd_bradfield}

El comportamiento paramétrico de la asimetría azimutal ha sido recientemente explorado en la literatura de la Colaboración Auger, destacándose el trabajo de F. Bradfield \cite{bradfield2022} aplicado a la señal total del Detector de Superficie (SD). Es pertinente contextualizar los resultados obtenidos con el UMD frente a dichos estudios para evidenciar la naturaleza divergente de las componentes de la cascada.

En su análisis, Bradfield propone una parametrización de la asimetría acoplada radialmente a la Función de Distribución Lateral (LDF), donde la modulación angular es modulada por un factor dependiente de la distancia al núcleo. Al evaluar dicha parametrización (ej. Figura 4.5 de \cite{bradfield2022}) bajo las restricciones geométricas de nuestro entorno de control ---fijando el radio a $r = 450$~m en el Anillo Denso---, la dependencia radial se anula, colapsando analíticamente en el ajuste armónico puramente azimutal $S(\phi) = \langle S \rangle (1 + A_1 \cos\phi)$ utilizado a lo largo de este capítulo. Esta convergencia paramétrica valida la robustez de nuestro marco geométrico. 




Sin embargo, al analizar la dependencia de la asimetría con la evolución longitudinal de la cascada, surge una discrepancia física fundamental. En los análisis realizados sobre el SD, se reporta que la magnitud de la asimetría experimenta un decrecimiento a medida que aumenta la profundidad del máximo ($X_{max}$). Este comportamiento es directamente opuesto a la dinámica observada empíricamente en el UMD en las simulaciones, donde frentes de cascada más profundos (primarios livianos o de mayor energía) inducen una mayor amplitud $A_1$.

---

Al analizar la dependencia de la asimetría con la evolución longitudinal de la cascada, surge una divergencia fenomenológica entre los detectores. Para el SD, Bradfield reporta que la magnitud de la asimetría $A_1$ disminuye a medida que aumenta el $X_{max}$ (lluvias más profundas). Por el contrario, los resultados obtenidos con el UMD muestran la tendencia opuesta: frentes de cascada más profundos inducen una mayor amplitud de asimetría muónica.



Esta discrepancia se explica mediante la competencia de los mecanismos de producción y transporte. El SD mide una señal dominada por la componente electromagnética ($e^\pm, \gamma$). Debido a que la longitud de radiación en el aire es pequeña, la componente EM es extremadamente sensible a las variaciones en el espesor atmosférico atravesado. Como la zona tardía recorre una mayor distancia en un medio denso, la atenuación exponencial castiga severamente su densidad. Este proceso es tan eficiente que, en ángulos cenitales bajos y medios, la asimetría EM supera a la muónica debido a la gran sección eficaz de interacción de los procesos de cascada electromagnética (bremsstrahlung y creación de pares). Sin embargo, a medida que la lluvia penetra más profundamente (mayor $X_{max}$), el frente EM se vuelve más "viejo" y su gradiente de atenuación local se estabiliza cerca del máximo, reduciendo la diferencia relativa entre zonas y, por ende, disminuyendo el valor de $A_1$ en el SD.

En el UMD, la situación es distinta. Al estar blindado contra la componente EM, el detector registra únicamente muones, los cuales poseen una sección eficaz de interacción mucho menor y una gran capacidad de penetración. La asimetría muónica no está dictada por la atenuación local, sino por la divergencia geométrica integral desde el punto de producción ($\Delta d / D$). Un $X_{max}$ profundo (característico de protones o eventos de alta energía) sitúa el máximo de producción muónica mucho más cerca del suelo, reduciendo drásticamente la distancia de propagación $D$. Al reducirse este "brazo de palanca" geométrico, el peso relativo de la diferencia de caminos ($\Delta d / D$) aumenta significativamente. 

En conclusión, mientras que el SD pierde poder resolutivo ante lluvias profundas debido a la estabilización de su componente dominante, el UMD ve potenciada su señal de asimetría por un efecto de proximidad geométrica. Este desacople consolida al parámetro $A_1$ del UMD como un trazador de masa más robusto y directo, ya que su sensibilidad aumenta precisamente allí donde la componente electromagnética comienza a saturar su capacidad de discriminación.

---


Esta aparente contradicción se resuelve al considerar la naturaleza de las partículas detectadas y los mecanismos físicos que dominan cada observable, un aspecto que carece de justificación rigurosa en tratamientos previos del SD. Las estaciones de superficie miden una señal fuertemente dominada por la componente electromagnética ($e^\pm, \gamma$). La asimetría de esta componente está gobernada primordialmente por el gradiente de atenuación longitudinal ($\partial N_e / \partial X$). Para primarios livianos con un $X_{max}$ profundo, la cascada alcanza el nivel del suelo cerca de su punto de máximo desarrollo. En esta región de la curva de Gaisser-Hillas, el gradiente longitudinal es mínimo (la derivada tiende a cero), lo que suprime drásticamente la diferencia de atenuación entre la región temprana y tardía, lavando la asimetría electromagnética.

En estricto contraste, el UMD (blindado por $2.3$~m de tierra) es insensible a la componente electromagnética y registra un remanente puramente muónico. Como se derivó en secciones anteriores, la modulación azimutal de los muones no depende del gradiente de atenuación local, sino de la divergencia geométrica integral desde el punto de producción ($\Delta d / D$). Un $X_{max}$ profundo minimiza la distancia cinemática total $D$, amplificando severamente el peso relativo de las proyecciones espaciales y, consecuentemente, disparando el valor de la asimetría $A_1$.

Este desacople fenomenológico evidencia una limitación intrínseca en el uso de la asimetría del SD como trazador de composición a altas inclinaciones. La medición pura del UMD demuestra ser un observable superior y directo: al estar libre de la supresión de asimetría que sufre la componente electromagnética cerca de su $X_{max}$, el parámetro $A_1$ de la densidad de muones maximiza su poder resolutivo, consolidando al \textit{Sweet Spot} ($40^\circ \le \theta \le 55^\circ$) como una herramienta robusta para la discriminación de la masa primaria.
