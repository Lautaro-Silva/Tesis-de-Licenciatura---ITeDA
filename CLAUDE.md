...
**Language: Spanish only in the thesis corpus itself, English everywhere else.** `Tesis - Latex/` is written in Spanish (it's a FCEN-UBA thesis) and any text added to it — new prose, revised sections, figure captions, LaTeX comments — must stay in Spanish to match. Everywhere else — chat replies, review notes and reports, `.md` files, `.html` artifacts, code comments, commit messages — use English, even when discussing or quoting Spanish thesis text (quote the Spanish verbatim when citing it, but write the surrounding analysis in English). `Notas  - Latex/` is the one ambiguous case: it's informal working notes the author already writes in a Spanish/English mix, so match whatever language the surrounding note is already in rather than forcing a switch.
...
- **Background/worktree sessions: land deliverables in `claude_work/` with a plain copy, not just inside the worktree.** A session running isolated in `.claude/worktrees/<name>/` (the harness's own convention for background jobs) only sees its own worktree — the user works through JupyterLab, which only sees the main checkout and cannot see anything under `.claude/`. When a deliverable (a report, analysis note, script) is ready for review, copy the finished files directly into a new, clearly-named subfolder under `claude_work/` in the main checkout (e.g. `claude_work/<topic>_v2/`) using plain `cp` — the harness blocks the Write/Edit tools from touching the shared checkout while a session is worktree-isolated, but a `cp`/`Bash` copy out to the main path is not blocked and does not require exiting the worktree. This is a separate step from git: making files visible this way needs no commit/push, and git actions still require the user's explicit permission per the rules above. Don't overwrite deliverables already sitting in `claude_work/`; add a new versioned subfolder alongside them (e.g. `_v2`) so the prior version stays intact for comparison.
