...
Sharpened by `sd_umd_synthesis.md` Part 5/6 into a specific test of the "instrumental
...
### 5. Compare each muon's actual ground-arrival direction to Cazón's production-kinematics-only prediction (needs per-muon production data — not confirmed available)

Motivated by `sd_umd_synthesis.md` Part 4: in-flight atmospheric multiple Coulomb
scattering (Grieder 2010, §3.1 — new bibliography for this review, not previously cited
by either GAP note or the thesis) is treated as additive to production-kinematics
scattering, and has the qualitatively right sign to help explain the SD's true-flux
inversion (longer late-region path $\Rightarrow$ more scattering $\Rightarrow$
multiplicatively boosts the late/early ratio), but the standard Highland RMS estimate
puts it about an order of magnitude too small on its own — quantified in the script's
§6. That estimate only captures the Gaussian core of the scattering distribution, not
the wide-angle Molière tail, which is exactly the kind of rare, high-leverage population
this whole investigation has already found naive bulk estimates miss (§5 of the script).
Rather than build a fourth analytic toy model of that tail, the direct empirical test is
cheaper and more reliable: for each simulated muon, compare its actual ground-arrival
direction to what Cazón's production-kinematics-only formula (§1 of the script) predicts
for that same muon's production height/angle/energy. The gap between the two is the
propagation-scattering contribution, measured rather than modeled — and whether it's
larger in the late region the way the hypothesis needs is then a direct read of existing
simulation output. This needs the same per-muon production data as check #3 (energy at
minimum; ideally also production height and angle) — not confirmed available in the
current ADST, same caveat as above.
...
2. In parallel, a short check (in the Auger Offline environment, not this venv) of
   ...
   variable at the SD station (needed for check #0), (b) per-particle energy/entry-face
   information at the SD/UMD boundary (needed for checks #2 and #3), and (c) per-muon
   production height/angle/energy (needed for check #5) — to scope all four honestly
   before proposing them as "cheap."
3. Only after that: decide whether #0/#2/#3/#5 are a light reprocessing pass or need new
