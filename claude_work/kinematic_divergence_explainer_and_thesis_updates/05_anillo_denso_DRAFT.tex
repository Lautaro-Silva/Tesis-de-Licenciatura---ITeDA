...
% "Tesis - Latex/capitulos/05_anillo_denso.tex", generada en la revision de
% claude_work/gap_notes_asimetrias_review_v4/ (ver ese folder para el contexto
% completo). Cambios acotados a la Seccion "Dependencia de A1 con la energia"
% y "Contraste fenomenologico con el SD": correccion de la atribucion del
% mecanismo (era atenuacion, debe ser divergencia cinematica 1/D) y agregado
% de la verificacion cuantitativa (tasa de elongacion de Xmax -> Delta D ->
% Delta A1 predicho vs. medido). No compila de forma independiente fuera de
% "Tesis - Latex/". Compilado y verificado sin errores contra el arbol real
% antes de guardarse aqui como borrador.
%
% !TEX root = ../main.tex
...
La hipótesis de trabajo propuesta para explicar este comportamiento físico radica en el corrimiento de la profundidad del máximo de desarrollo de la cascada ($X_{\mathrm{max}}$). Teóricamente, un aumento en la energía del primario implica que la lluvia penetra más profundamente en la atmósfera antes de alcanzar su número máximo de partículas, acercando el $X_{\mathrm{max}}$ al nivel del suelo y, por lo tanto, reduciendo la distancia efectiva de producción $D$ entre el origen de los muones y los detectores. Es importante notar que este no es el mismo mecanismo que domina en el SD: la asimetría de la componente electromagnética, gobernada por la atenuación, en realidad se \textit{reduce} al acercarse al máximo de desarrollo, ya que depende del gradiente longitudinal local de la cascada (Sección \ref{subsec:contraste_sd_bradfield}). La asimetría puramente muónica que domina en el UMD, en cambio, está regida por el mecanismo cinemático de divergencia del flujo, $A_\mu \approx 2r\sin\theta/D$ (Ec.~\ref{eq:asym_mu_cinematica}), que \textit{crece} al disminuir $D$ —de signo opuesto al de la componente EM frente a la misma variación de $X_{max}$. Esta distinción se desarrolla y se verifica cuantitativamente, contrastando la predicción de la Ec.~\ref{eq:asym_mu_cinematica} contra la tasa de elongación estándar de $X_{max}$, en la Sección \ref{subsec:contraste_sd_bradfield}, donde se obtiene un acuerdo de orden de magnitud con el incremento aquí reportado.
...
Esta hipótesis admite una verificación cuantitativa directa, más allá del argumento cualitativo anterior, combinándola con la tasa de elongación estándar de $X_{max}$, $dX_{max}/d\log_{10}E \approx 55$–$60~\mathrm{g/cm^2}$ por década \cite{yushkov2019mass}. Utilizando un modelo de atmósfera isotérmica estándar para convertir esta variación de $X_{max}$ en un corrimiento de la distancia de producción $D$ (evaluado en $\theta = 50^\circ$, punto medio de la banda angular $[40^\circ,60^\circ]$ analizada en la Sección \ref{subsec:dependencia_energia}, con $D_{\mathrm{ref}} = 7.5$~km, valor central citado tanto por Cazón \cite{Cazon2012} como por Bertou y Billoir \cite{GAP2000_017}), la media década en $\log_{10}(E/\mathrm{eV})$ de la Figura \ref{fig:a1_vs_energia} corresponde a $\Delta X_{max} \approx 28$–$30~\mathrm{g/cm^2}$, es decir $\Delta D \approx -460$ a $-500$~m. Sustituyendo en la Ec.~\ref{eq:asym_mu_cinematica} se predice un incremento $\Delta A_1 \approx +0.006$ a $+0.007$ (o $\Delta A_1 \approx +0.009$ a $+0.010$ si se evalúa el cociente exacto $(d_{temprano}/d_{tardio})^2$ en lugar de la aproximación de Taylor de primer orden), frente al $\Delta A_1 \approx 0.015$ efectivamente medido en la Sección \ref{subsec:dependencia_energia}. El mecanismo propuesto reproduce correctamente tanto el \textbf{signo} como el \textbf{orden de magnitud} del efecto, con un factor residual de $\sim 1.5$–$2.5$ sin explicar. Este remanente podría originarse en que la tasa de elongación específica de la profundidad de producción \textit{muónica} difiera de la utilizada aquí (calculada para el máximo del desarrollo electromagnético), en una contribución adicional de $\mathcal{A}_{geo}$ (Sección \ref{subsubsec:divergencia}, que también depende de $D$ a través del ángulo local de incidencia y se suma al mecanismo puramente cinemático aquí evaluado), o en las fluctuaciones evento a evento de $X_{max}$ dentro de cada bin de energía reconstruida, no consideradas en esta estimación puntual. Este acuerdo, aunque parcial, es suficiente para elevar la interpretación de la Sección \ref{subsec:dependencia_energia} de una hipótesis puramente cualitativa a un mecanismo cuantitativamente respaldado, sin que esto implique que se trate de la única contribución en juego.
