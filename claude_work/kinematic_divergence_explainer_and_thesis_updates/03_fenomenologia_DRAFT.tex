...
La existencia de este mecanismo sugiere, a primera vista, una oportunidad experimental vinculada a los umbrales de detección: si la ganancia cinemática exponencial es la responsable del exceso tardío, un instrumento con un corte de energía estricto —como el blindaje de $2.3$~m de tierra del UMD, que impone $E_\mu \gtrsim 1\,\mathrm{GeV}/\cos\theta$— actuaría como un filtro que la suprime, preservando la asimetría positiva de atenuación. Conviene, sin embargo, evaluar esta expectativa de manera cuantitativa antes de adoptarla, y el resultado no la respalda.

Para ello es necesario promediar la Ec.~\ref{eq:asym_kinematic_ratio} sobre el espectro energético real de muones, lo cual requiere dos precauciones. Primero, la magnitud físicamente relevante no es el promedio del cociente $\langle S_{tardio}/S_{temprano}\rangle$, sino el cociente de los flujos integrados en cada región: la región temprana exige el ángulo \textit{mayor} y por lo tanto está exponencialmente más suprimida a \textit{toda} energía, de modo que pesar ambas regiones con el mismo espectro de producción $N(E)\propto E^{-\gamma}$ elimina precisamente el efecto de selección que se quiere medir. Segundo, la distribución angular de la Ec.~\ref{eq:cazon_dNdOmega}, correctamente normalizada a partir de $dN/dp_t \propto p_t\exp(-p_t/Q)$, incorpora un prefactor $k^2 \equiv (E/cQ)^2$: éste se cancela en el cociente tardío/temprano a energía \textit{fija}, pero no bajo una integral espectral, ya que expresa que un muón energético concentra su emisión en un ángulo sólido $\sim 1/k^2$ y contribuye por lo tanto con una densidad mayor en la dirección en que efectivamente se emite.

Evaluando el cociente de flujos integrados con ambas correcciones, en el punto de referencia $r = 1200$~m y $\theta = 35^\circ$ (con $D = 7.5$~km, $Q = 0.2$~GeV y $\gamma = 2.6$), se obtiene un resultado contrario a la expectativa del filtro cinemático: la amplitud predicha \textit{disminuye} al elevar el umbral, pasando de $A_1 \simeq +0.16$ para un umbral tipo SD ($E_{th} = 0.155$~GeV, el mínimo de la simulación) a $A_1 \simeq +0.02$ para el umbral efectivo del UMD ($\simeq 1.22$~GeV a este ángulo), y cruzando a valores negativos para umbrales superiores a $\sim 2$~GeV. Es decir: el mecanismo cinemático, evaluado consistentemente, predice que el UMD debería exhibir una asimetría \textit{menos} positiva que el SD, cuando lo que se mide en las simulaciones es exactamente lo contrario (Capítulos \ref{sec:cap_anillo_denso} y 6). La divergencia cinemática es, por lo tanto, un mecanismo real y correctamente derivado, pero no constituye por sí sola la explicación del contraste observado entre ambos detectores, y la imagen del umbral del UMD como ``filtro'' que elimina una población responsable del exceso tardío no sobrevive a una evaluación cuantitativa. El origen físico de dicho contraste se retoma en el Capítulo 6.
