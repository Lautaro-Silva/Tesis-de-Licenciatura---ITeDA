...
**Follow-up (`sd_umd_synthesis.md`):** the exact-zero VEM result generalizes beyond this idealized-uniform-flux calculation — it holds region by region for *whatever* the true incident flux is, since aperture and mean-chord length are reciprocal at every angle individually, not just on average. That means the muon VEM signal is a clean, undistorted tracer of the true flux ratio, which lets the true-flux $A_1$ be backed out of Table 2's reported count ($\approx-0.13$, more inverted than the raw count's $-0.10$) and gives a falsifiable prediction (an isolated muon-only VEM signal should show a *larger* inversion than the muon count). It also motivated checking whether kinematic divergence survives being properly weighted over a realistic muon spectrum rather than evaluated pointwise — it does not (see that file, Part 2): the spectrum-weighted term stays early-favoring over any physically bounded energy range, retracting a hypothesis floated in conversation that a high-energy tail above $E^*$ might explain the inversion. The physical origin of the SD's true-flux inversion is not identified by any mechanism checked so far, in that file or this one.
...
Full derivation, exact geometry, $D$-scan, Luce cross-check, tank/threshold calculations, the implied-true-flux back-out, and the spectrum-weighted kinematic-term check (§§4-5, added for `sd_umd_synthesis.md`): `verificacion_eq9_kinematic_divergence.py` (same folder; run with the repo's `venv`: `venv/bin/python claude_work/gap_notes_asimetrias_review/verificacion_eq9_kinematic_divergence.py`; needs `numpy`, `scipy`; `sympy` optional, not installed in this repo's venv).
